...
from .config import ItemConfig, JenkinsInstance
...
class JenkinsClient:
    ...
    async def trigger(self, item: ItemConfig, branch: str) -> str:
        """触发构建, 返回 queue item URL"""
        path = job_url_path(item.job)
        if item.parameterized:
            url = f"{self.base}/{path}/buildWithParameters"
            params = item.build_params or None
        else:
            url = f"{self.base}/{path}/build"
            params = None
